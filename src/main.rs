...
async fn bench_point_lookup(
    ...
    let sample: Option<(String, String, i64)> = sqlx::query_as(
        "SELECT public_key, h3_cell, epoch::bigint FROM breadcrumbs ORDER BY created_at DESC LIMIT 1",
    )
...
async fn bench_trajectory(
    ...
    let sample_records: Vec<(String, String, i64)> = sqlx::query_as(
        "SELECT public_key, h3_cell, epoch::bigint FROM breadcrumbs ORDER BY created_at DESC LIMIT 20",
    )
    .fetch_all(&state.pg)
    .await
    .unwrap_or_else(|e| {
